...
class HomeController extends Controller
{
    ...
    {   
        ...
        if(Auth::user()->role =='client'){
            ...
            $today=now();
            $rendez_vous = DB::table('rendez_vouses')->where('date','>=',$today)->orderBy('date','asc')->get();
            $rendez_vous_history = DB::table('rendez_vouses')->where('date','<',$today)->orderBy('date','asc')->get();
            $client_user_table= User::find($client->client_id);
            if($client!=null)
                return view('reservation.landing',['client'=> $client,'etablissement'=> $etablissement,'rendez_vouss'=>$rendez_vous,'rendez_vous_history'=>$rendez_vous_history,'client_user_table'=>$client_user_table]);
            else
                return view('welcome');
            }
